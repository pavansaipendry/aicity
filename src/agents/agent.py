...
class Agent(BaseModel):
    ...
    # ── Newborn development ────────────────────────────────────────────────
    # comprehension_score: 0–100. Reaches 100 → ready to graduate.
    # Grows each day based on how present and engaged the teacher is.
    # A close teacher (high bond) = fast growth. Absent teacher = slow growth.
    comprehension_score: int = 0

    # The teacher who is guiding this newborn. Set at birth.
    # None if no teacher is alive in the city.
    ...
    def is_ready_to_graduate(self) -> bool:
        """Newborn has learned enough to choose their role."""
        return self.role == AgentRole.NEWBORN and self.comprehension_score >= 100
    ...
    def __repr__(self) -> str:
        ...
        if self.role == "newborn":
            extra = f" | Comprehension: {self.comprehension_score}%"
        return (
            f"{emoji} Agent {self.name} | Role: {self.role} | "
            f"Tokens: {self.tokens} | Age: {self.age_days:.1f} days{extra}"
        )